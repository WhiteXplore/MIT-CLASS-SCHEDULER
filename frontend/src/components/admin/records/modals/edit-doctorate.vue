        ...
        <div class="p-5 w-[25vw] space-y-3">
          ...
          <div class="w-full space-y-2 text-left flex flex-col">
            ...
            <input
              ...
              required
              class="w-full border px-4 py-3 border-gray-600 rounded-md text-md text-gray-800"
              placeholder="Enter doctorate type"
            />
          </div>
...
export default {
  name: "EditDoctorateProgramsPage",
  components: {
    icon,
  },
  props: {
  ...
  methods: {
    async submitData() {
      const formElement = this.$refs.doctorateForm;
      ...
      try {
        ...
        this.$emit("close");
        toast.success("Doctorate program updated successfully!");
      } catch (error) {
        toast.error("Failed to update doctorate program. Please try again.");
      }
    },
  },
};
