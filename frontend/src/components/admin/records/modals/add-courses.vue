  ...
  >
    <div class="rounded-[15px] shadow-lg">
      ...
      >
        <!-- Header -->
        <div
          class="w-full p-5 py-3.5 bg-green-600 text-white rounded-t-[15px] flex justify-between items-center border-b shadow"
        >
          <div class="flex gap-1 items-center">
            <icon :name="'add-students'" />
            <h1 class="font-bold tracking-wide text-lg">Add Courses</h1>
          </div>
          <icon
            :name="'circle-close3'"
            @click="$emit('close')"
...
export default {
  ...
  computed: {
    ...
    // ✅ Always true/false depending on description content
    isRequisiteDisabled() {
      return (
