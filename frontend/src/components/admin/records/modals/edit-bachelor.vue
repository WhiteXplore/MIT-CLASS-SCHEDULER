...
export default {
  ...
  watch: {
    // Watch for changes to prop and update local form
    bachelorProgram: {
      immediate: true,
      handler(newVal) {
        if (newVal) {
  ...
  methods: {
    async submitData() {
      ...
      try {
        ...
        this.$emit("close");
        toast.success("Bachelor program updated successfully!");
      } catch (error) {
        toast.error("Failed to update bachelor program. Please try again.");
      }
    },
  },
};
