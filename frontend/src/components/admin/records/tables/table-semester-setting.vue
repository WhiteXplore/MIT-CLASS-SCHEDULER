<template>
  <div v-if="isTable" class="">
    <div class="text-sm flex justify-between">
      ...
      >
        ...
        <span class="font-medium text-sm">Add Term</span>
      </div>
    </div>
            ...
            >
              ...
              >
                <tr>
                  ...
                  <th class="px-4 py-3 text-left font-normal">School Year</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-2 text-left">{{ item.semester }}</td>
                  <td class="px-4 py-2 text-left">
                    {{ item.school_year_since }} -
  ...
  <AddSemesterSettings
    v-if="isAdd"
    @close="closeView"
    @refresh="loadSemesterSettings"
  />
  <!-- <EditSemesterSettings
    v-if="showEditModal && selectedSemesterSetting"
    :semesterSettingData="selectedSemesterSetting"
    @close="closeModal"
    @refresh="loadSemesterSettings"
  /> -->
...
import AddSemesterSettings from "../modals/add-semester-settings.vue";
// import EditSemesterSettings from "../modals/edit-semester-settings.vue";
...
export default {
  ...
  data() {
    return {
      ...
      recordToDelete: null,
      selectedSemesterSetting: null,
      showEditModal: false,
    };
  },
  computed: {
    ...mapState(useFetchDataStore, ["semesterSettings"]),
    filteredData() {
      const query = this.searchQuery.toLowerCase();
      return this.semesterSettings.filter((item) =>
        `${item.semester ?? ""} ${item.school_year_since ?? ""} ${
          item.school_year_effective ?? ""
        } ${item.term_status ?? ""}`
          .toLowerCase()
          .includes(query)
  ...
  methods: {
    async loadSemesterSettings() {
      const store = useFetchDataStore();
      await store.fetchSemesterSettings();
    },
    toggleUploadData() {
    ...
    toggleEdit(item) {
      this.selectedSemesterSetting = item;
    ...
    confirmDelete() {
      if (
        !this.recordToDelete ||
        isNaN(this.recordToDelete.semester_settings_id)
      ) {
        toast.error("Invalid Semester Setting ID.");
        return;
      }
    ...
    closeModal() {
      ...
      this.selectedSemesterSetting = null;
    },
    handleBackToTable() {
      this.isEdit = false;
      this.isAdd = false;
