        ...
        >
          <thead>
            <tr class="bg-blue-800 text-white uppercase tracking-wide">
              <th class="p-3 text-center rounded-tl-xl font-normal">Time</th>
              <th
          ...
          <tbody>
            ...
            >
              <td
                class="bg-gray-100 text-gray-700 p-2 text-center font-semibold w-[6vw] border border-gray-100"
              >
                {{ timeSlot }}
              </td>
              ...
              >
                <ul class="space-y-2 text-left">
                  <template v-if="getSlotsForDayTime(day, timeSlot).length > 0">
                    <li
                      v-for="(slot, idx) in getSlotsForDayTime(day, timeSlot)"
                      :key="idx"
...
export default {
    ...
    filteredData() {
      ...
      return data.filter(
        (item) =>
          ...
          "not available".includes(q) || // or from your status logic
  ...
  methods: {
    ...
    getSlotsForDayTime(dayName, timeRange) {
      ...
      this.roomSchedules = schedulers.flatMap((entry) => {
        ...
        return days.map((dayName) => {
          ...
          return {
            ...
            date,
            room: entry.room?.room_name || "Unknown",
