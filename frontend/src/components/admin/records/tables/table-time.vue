<template>
  <div v-if="isTable" class=" ">
    <div class="text-sm flex justify-between">
      <div class="text-[13px] text-text mt-4 font-regular">Pages / Time</div>
      ...
      >
        ...
        <span class="font-medium text-sm">Add Time</span>
      </div>
    </div>
  ...
  <AddTime v-if="isAdd" @close="closeView" @refresh="loadTime" />
  <editTime
    v-if="showEditModal && selectedTime"
...
import AddTime from "../modals/add-time.vue";
import editTime from "../modals/edit-time.vue";
...
export default {
  ...
  computed: {
    ...mapState(useFetchDataStore, ["time"]),
    filteredData() {
  ...
  methods: {
    ...
    confirmDelete() {
      if (!this.recordToDelete || isNaN(this.recordToDelete.time_id)) {
        toast.error("Invalid time ID.");
        return;
      }
      ...
      axios
        ...
        .then(() => {
          ...
          audio.play();
          this.loadTime(); // refresh the list from backend
